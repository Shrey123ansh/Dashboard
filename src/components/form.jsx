      ...
      const checkoutHandler = async () => {
            ...
            const options = {
                ...
                "handler": async (response) => {
                  ...
                  const res = await axios.post("http://localhost:5000/api/paymentverification" ,response);
                  console.log(res);
                  if(res.status=== 200)
                  {
...
<div className="conta" style={{position : "relative"}}>
  ...
  <input type="email" name="email" placeholder="Email" required={true} onChange={(e) => setemail(e.target.value)} />
      <input type="number" name="phone" placeholder="Mobile Number" required={true} onChange={(e) => setphone(e.target.value)} />
    <input type="text" name="rollNo" placeholder="Roll Number (for nitw students)" onChange={(e) => setrollNo(e.target.value)} />
    <span style={{fontSize : "1.5rem"}}>enter number of tickets</span>
<input type="number" name="count" placeholder="Number of Tickets" required={true} value={count} onChange={handleChange}/>
 
  <div id="linksParent">
    Tickets Left : {tickets} <br/>
   Total amount to be paid : {count===0?0:count*500}
  </div>
  
  <div id="button">
    <button className="btt" onClick={() => checkoutHandler()}> PAY</button>
